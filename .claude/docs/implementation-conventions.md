...
@example(#{
  ...
  meta: #{ respondedAt: RespondedAt.fromISO("2025-01-01T12:00:00Z") },
})
model BadRequestException { ... }
```

`utcDateTime` を継承した日時スカラー（`RespondedAt` / `CreatedAt` / `UpdatedAt` など）は、
文字列リテラルではなく自分自身の `fromISO()` を使う。文字列を直接渡すと型エラーになる。

```tsp
// スカラー定義側
@example(RespondedAt.fromISO("2025-01-01T12:00:00Z"))
scalar RespondedAt extends utcDateTime;

// モデルの @example 内で使う場合
@example(#{ respondedAt: RespondedAt.fromISO("2025-01-01T12:00:00Z") })
```
...
このリポジトリでは全サービス `3.1.0` に統一する。

`3.1.0` は JSON Schema draft 2020-12 に完全準拠しており、以下の利点がある：

- `nullable: true`（3.0 独自拡張）が消え、`type: [string, "null"]` で表現される
- `example`（単数）ではなく `examples`（複数形・JSON Schema 標準）で出力される

新サービスを追加するときも必ず `3.1.0` を指定し、既存サービスと出力形式を揃える。
